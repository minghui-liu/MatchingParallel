...
void marginalize(Matrix d_G1, Matrix d_G2t, double sigma, Matrix d_Y) {
	...
	repmat(d_G1_col, 1, d_G1.width, d_D1);
	repmat(d_G2t_row, d_G2t.width, 1, d_D2);
	matSub(d_D1, d_D2, d_D);
		
	// exp((-d.*d)./sigma)	
	int offset_D = y * d_D.width + x;
	*(d_D.elements + offset_D) = exp(-(*(d_D.elements + offset_D)) * (*(d_D.elements + offset_D)) / sigma); 
	// write to Y
	int offset_Y = y * d_Y.width + x;
	*(d_Y.elements + offset_Y) += *(d_D.elements + offset_D);
}
...
void graphMatching(Matrix G1, Matrix G2, double sigma, int numberOfMatches, Matrix X, Matrix Z, Matrix Y) {
	...
	*****************************************************************************************/
	...
	/* debug please remove */
	Matrix G2t;
	G2t.width = d_G2t.width;
	G2t.height = d_G2t.height;
	size = G2t.width * G2t.height * sizeof(double);
	G2t.elements = (double*)malloc(size);
	// read d_G2t from device memory
	err = cudaMemcpy(G2t.elements, d_G2t.elements, size, cudaMemcpyDeviceToHost);
	printf("Copy d_G2t off of device: %s\n",cudaGetErrorString(err));
	printMatrix(G2t);
	...
	err = cudaMemcpy(d_Y.elements, Y.elements, size, cudaMemcpyHostToDevice);	
